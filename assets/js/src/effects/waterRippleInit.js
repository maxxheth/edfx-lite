...
const waterRippleInit = homeSections => {
    const homeSectionCanvasSets = homeSections.map((homeSection, homeIndex) => {
        ...
        homeSectionCanvas.setAttribute(
            "width",
            getComputedStyle(homeSection).width
        );

        homeSectionCanvas.setAttribute(
            "height",
            getComputedStyle(homeSection).height
        );

        return {
            homeSectionCanvas: document.getElementById(homeSectionCanvas.id),
            homeSectionBackgroundImgUrl: getComputedStyle(homeSection)
                .backgroundImage,
    ...
    homeSectionCanvasSets.forEach(canvasSet => {
        ...
        makeWaterRipples(homeSectionCanvas)(homeSectionBackgroundImgUrl)({

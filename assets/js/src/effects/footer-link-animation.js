...
const footerLinkAnimation = () => {
	...
	const edfxFooterLinks = [...edfxFooter.querySelectorAll("a")].filter(
		(link) => link.classList.contains("edfx-footer__link"),
	);

	const edfxFooterFirstHalf =
		edfxFooter.querySelectorAll(".edfx-footer__half")[0];

	const edfxFooterSecondHalf =
		edfxFooter.querySelectorAll(".edfx-footer__half")[1];

	const edfxSocialMediaLinks = [
		...document.querySelectorAll(".edfx-footer__social-media-link"),
	];
	...
	edfxFooterLinks.forEach((link, linkIndex) => {
		...
		link.addEventListener("mouseenter", () => {
			...
			const otherLinks = edfxFooterLinks.filter(
				(footerLink) => footerLink !== link,
			);

			otherLinks.forEach((otherLink) => otherLink.classList.add("edfx-beige"));

			edfxSocialMediaLinks.forEach((smLink) =>
				smLink.classList.add("edfx-blue"),
			);
		...
		link.addEventListener("mouseleave", () => {
			...
			otherLinks.forEach((otherLink) =>
				otherLink.classList.remove("edfx-beige"),
			);

			edfxSocialMediaLinks.forEach((smLink) =>
				smLink.classList.remove("edfx-blue"),
			);

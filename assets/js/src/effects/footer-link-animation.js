...
const footerLinkAnimation = () => {
    ...
    const edfxFooterLinks = [...edfxFooter.querySelectorAll("a")].filter(link =>
        link.classList.contains("edfx-footer__link")
    );

    const edfxFooterFirstHalf = edfxFooter.querySelectorAll(
        ".edfx-footer__half"
    )[0];

    const edfxFooterSecondHalf = edfxFooter.querySelectorAll(
        ".edfx-footer__half"
    )[1];
    ...
    edfxFooterLinks.forEach((link, linkIndex) => {
        ...
        link.addEventListener("mouseenter", () => {
            ...
            otherLinks.forEach(otherLink =>
                otherLink.classList.add("edfx-beige")
            );

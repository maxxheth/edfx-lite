const makeWaterRipples = canvas => backgroundImgUrl => ({width, height} = {}) => {
    ...
    canvas.setAttribute('width', Number(width.match(/\d+/)[0]))

    canvas.setAttribute('height', Number(height.match(/\d+/)[0]));
    ...
    const httpRegex = /(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])?/;

...
const canvasDoodle =
	...
	({ canvasWidth = 0, canvasHeight = 0 } = {}) => {
		...
		const redraw = (canvasParams = {}) => (context) => (clickRecord = {}) => {
			...
			const { strokeStyle, lineJoin, lineWidth } = canvasParams;
			...
			const { clickXEvents, clickYEvents, clickDragEvents } = clickRecord;
		...
		const manageClickRecord =
			(clickRecord = {}) =>
			({ mouseX, mouseY, mouseDrag }) => {
				if (!Object.entries(clickRecord).length > 0) {
					...
					if (!mouseDrag) {
						clickRecord = recordClicks(clickRecord)({
							clickX: mouseX,
							clickY: mouseY,
						});
					} else {
			...
			(event) => {
				...
				if (canvasState.clickRecord) {
					redraw({
						...
						lineJoin: "round",
						lineWidth: 5,
					})(context)(canvasState.clickRecord);
				}
			},
			false,
		);
